...
import com.uknowzxt.beans.factory.support.BeanDefinitionReaderUtils;
...
public class ConfigBeanDefinitionParser /*implements BeanDefinitionParser*/ {
	...
	private void parseAspect(Element aspectElement, BeanDefinitionRegistry registry) {
		...
		for (int i=0;i<eleList.size();i++) {
			...
			if (isAdviceNode(ele)) {//判断是不是advice, (pointcut不会进入)
				if (!adviceFoundAlready) {
					adviceFoundAlready = true;
					if (!StringUtils.hasText(aspectName)) {
						return;
					}
					beanReferences.add(new RuntimeBeanReference(aspectName));//<aop:aspect ref="tx">作为一个RuntimeReference
				}
	...
	private GenericBeanDefinition createAdviceDefinition(
		...
		Object pointcut = parsePointcutProperty(adviceElement);//<aop:before pointcut-ref="placeOrder" method="start" /> 中的placeOrder
		if (pointcut instanceof BeanDefinition) {
			...
			RuntimeBeanReference pointcutRef = new RuntimeBeanReference((String) pointcut);//创建切点的RuntimeBeanReference. id为placeOrder的切点
