...
public class CglibProxyFactory implements AopProxyFactory {
	...
	public Object getProxy(ClassLoader classLoader) {
		...
		try {
			...
			enhancer.setCallbackFilter(new ProxyCallbackFilter(this.config));//做了简化, 永远返回0 , 仅仅使用aop拦截器
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			List<Advice> chain = this.config.getAdvices(method/*, targetClass*/);//拿到当前方法适用的advice
			Object retVal;
			...
			if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
				...
				interceptors.addAll(chain);//aop联盟拦截器链
